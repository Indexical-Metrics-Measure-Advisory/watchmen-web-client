...
export const EnumEventBusProvider = (props: { children?: ((props: any) => React.ReactNode) | React.ReactNode }) => {
	...
	const [bus] = useState<EnumEventBus>({
		fire: (type: string, ...data: Array<any>): EnumEventBus => {
		...
		on: (type: string, listener: (...data: Array<any>) => void): EnumEventBus => {
		...
		off: (type: string, listener: (...data: Array<any>) => void): EnumEventBus => {
