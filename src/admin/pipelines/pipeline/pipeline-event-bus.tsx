...
export const PipelineEventBusProvider = (props: { children?: ((props: any) => React.ReactNode) | React.ReactNode }) => {
	...
	const [bus] = useState<PipelineEventBus>({
		fire: (type: string, ...data: Array<any>): PipelineEventBus => {
			emitter.emit(type, ...data);
			return bus;
		},
		once: (type: string, listener: (...data: Array<any>) => void): PipelineEventBus => {
			emitter.once(type, listener);
			return bus;
		},
		on: (type: string, listener: (...data: Array<any>) => void): PipelineEventBus => {
		...
		off: (type: string, listener: (...data: Array<any>) => void): PipelineEventBus => {
