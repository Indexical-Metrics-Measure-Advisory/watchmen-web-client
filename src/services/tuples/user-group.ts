...
export const listUserGroups = async (options: {
	...
	if (isMockService()) {
		...
		const response = await fetch(`${getServiceHost()}user_group/name?query_name=${search}`, {
			method: 'POST',
			headers: {
	...
	if (isMockService()) {
		...
		const fetchUsers = async (): Promise<Array<QueryUserForHolder>> => {
			...
			if (userIds && userIds.length > 0) {
				const response = await fetch(`${getServiceHost()}user/ids`, {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json'
					},
		...
		const fetchSpaces = async (): Promise<Array<QuerySpaceForHolder>> => {
			...
			if (spaceIds && spaceIds.length > 0) {
				const response = await fetch(`${getServiceHost()}space/ids`, {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json'
					},
